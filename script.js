...
// Tăng số lượng cánh hoa lên cực lớn để tạo bức tường che kín màn hình
const MAX_PETALS = 1000;
const petals = [];
...
let revealProgress = 0; // % Tiến trình mở màn hình video (0 -> 100%)
...
class TorrentPetal {
    ...
    draw() {
        ...
        let scaleX = Math.cos(this.rotationX);
        let scaleY = Math.sin(this.rotationY);
...
for (let i = 0; i < totalParticles; i++) {
    particles.push(new FlowParticle3D());
}
for (let i = 0; i < MAX_PETALS; i++) {
    petals.push(new TorrentPetal());
}
...
// BẬT CHUYỂN PHÁT CẢ VIDEO VÀ NHẠC ĐỒNG BỘ
function breakHeart() {
    isHeartBroken = true;
    revealProgress = 0;
    ...
    if (bgAudio) {
        ...
        bgAudio.play().catch(err => console.log("Lỗi trình duyệt chặn tự phát âm thanh:", err));
    }

    instruction.innerText = '✨ Nhấp / Chạm lần nữa để thu hồi Trái Tim ban đầu ✨';
}

// TẮT/TẠM DỪNG CẢ VIDEO VÀ NHẠC KHI THU HỒI
...
window.addEventListener('mouseup', () => {
    ...
    if (holdDuration < 350 && !isHeartBroken) {
        changeHeartColor();
    }
});
...
function animate() {
    ...
    if (!isHeartBroken) {
        ...
        if (dist < 160 && dist > 0) {
            let maxEvadeDist = 22; 
            let targetOffsetX = -(dx / dist) * maxEvadeDist;
            let targetOffsetY = -(dy / dist) * maxEvadeDist;

            heartOffsetX += (targetOffsetX - heartOffsetX) * 0.1;
            heartOffsetY += (targetOffsetY - heartOffsetY) * 0.1;
        } else {
